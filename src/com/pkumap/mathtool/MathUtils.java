...
{
	static public final double PI = 3.1415926535897932384626433832795d;
	...
	*/
	...
	static private class Cos
	{
		...
		{
			for (int i = 0; i < SIN_COUNT; i++)
				table[i] = (double) Math.cos((i + 0.5f) / SIN_COUNT * radFull);
			for (int i = 0; i < 360; i += 90)
				table[(int) (i * degToIndex) & SIN_MASK] = (double) Math.cos(i * degreesToRadians);
		}
	}
	*/
	
	/*
	static public final double sin(double rad)
	{
		return Sin.table[(int) (rad * radToIndex) & SIN_MASK];
	}

	static public final double cos(double rad)
	{
		return Cos.table[(int) (rad * radToIndex) & SIN_MASK];
	}

	static public final double sinDeg(double deg)
	{
		return Sin.table[(int) (deg * degToIndex) & SIN_MASK];
	}

	static public final double cosDeg(double deg)
	{
	...
	*/
	 ...
	 * double. This method will only properly ceil floats from -(2^14) to
	 * (double.MAX_VALUE - 2^14).
	 */
	...
	/**
	 * Returns the smallest integer greater than or equal to the specified
	 * double. This method will only properly ceil floats that are positive.
	 */
	 ...
	 * only properly round floats from -(2^14) to (double.MAX_VALUE - 2^14).
	 */
	 ...
	 * add by wudi
	 */
	static public boolean FEqual(double a, double b)
	{
